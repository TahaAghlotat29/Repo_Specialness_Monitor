...
from src.analytics.specialness import load_specialness_score
...
# ---------------------------------------------------------------------
st.set_page_config(
    page_title="EUR Repo Collateral Monitor",
    layout="wide",
...
selected = option_menu(
    menu_title=None,
    options=["Home", "Macro Overview", "Repo Tension", "Specialness "],
...
# ---------------------------------------------------------------------
def render_home():
    st.title("EUR Repo Collateral Monitor")
    st.markdown(
    "**Tracking collateral scarcity and specialness across EUR repo markets.**"
)
    st.markdown("---")

    st.markdown(
        """
        The **APP** and **PEPP** programmes still hold over €3tn of EUR sovereign and
        covered bonds, significantly reducing free float available for repo. As
        **quantitative tightening** progresses, this collateral is gradually
        returned to the market, reshaping repo tension and specialness across EUR
        markets.
        """
    )
    st.markdown("---")
    ...
    st.markdown(
        ...
        - **Specialness** — Bond-by-bond specialness risk scoring with adjustable
