...
def build_specialness_score():
    ...
    """Compute the specialness risk score for each bond in the universe"""
...
if __name__ == "__main__":
    df = build_specialness_score()

    print(f"\nBonds scored: {len(df)}")

    print("\nTop 10 highest specialness risk:")
    cols = [
        "isin", "issuer", "country", "maturity_bucket",
        "free_float_pct", "specialness_score", "risk_tier",
    ]
    print(df.nlargest(10, "specialness_score")[cols].to_string(index=False))

    print("\nRisk tier distribution:")
    print(df["risk_tier"].value_counts())

    print("\nScore breakdown (feature contributions — top 5):")
    breakdown_cols = [
        "isin", "score_free_float", "score_size",
        "score_age", "score_maturity", "specialness_score",
    ]
    print(df.nlargest(5, "specialness_score")[breakdown_cols].to_string(index=False))